...
from .models import Book, FlashCard, UserProfile, DictionaryEntry, Chapter, Conversation, Message
from .serializers import (BookCreateUpdateSerializer, FlashCardSerializer,
                          UserProfileSerializer, DictionaryEntrySerializer,
                          GoogleAuthSerializer, UserSerializer, ChapterDetailSerializer,
                          ChapterListSerializer, BookDetailSerializer, BookListSerializer,
                          BookUploadSerializer, ConversationSerializer, ConversationListSerializer,
                          MessageSerializer, TranslationRequestSerializer, TranslationResponseSerializer, TranslationSerializer)
...
class AuthViewSet(viewsets.GenericViewSet):
    ...
    def google_auth(self, request):
        ...
        if not user:
            return Response({'error': f'Failed to create user: {error}'}, status=status.HTTP_400_BAD_REQUEST)

        refresh = RefreshToken.for_user(user)
        access_token = refresh.access_token

        user_serializer = UserSerializer(user)
        return Response({'user': user_serializer.data, 'tokens': {
...
class BookViewSet(viewsets.ModelViewSet):
    ...
    def _validate_fb2_structure(self, root, namespace):
        ...
        for xpath, element_name in required_elements:
            if root.find(xpath, namespace) is None:
                raise ValueError(f"FB2 файл поврежден: отсутствует элемент {element_name}")
    ...
    def _extract_fb2_metadata(self, root, namespace):
        ...
        # Основные метаданные
        title_info = root.find('.//fb:title-info' if namespace else './/title-info', namespace)
        ...
        # Извлекаем авторов
        authors = []
        author_elements = title_info.findall('.//fb:author' if namespace else './/author', namespace)
        ...
        # Извлекаем аннотацию
        annotation_elem = title_info.find('.//fb:annotation' if namespace else './/annotation', namespace)
        description = ""
        if annotation_elem is not None:
            # Собираем текст из всех параграфов аннотации
            paragraphs = annotation_elem.findall('.//fb:p' if namespace else './/p', namespace)
            if paragraphs:
    ...
    def _process_fb2_section(self, section, namespace, order):
        ...
        if title_elem is not None:
            ...
            title_paragraphs = title_elem.findall('.//fb:p' if namespace else './/p', namespace)
            if title_paragraphs:
                chapter_title = ' '.join([p.text.strip() for p in title_paragraphs if p.text])
            else:
        ...
        for child in section:
            if child.tag.endswith('p') or (namespace and child.tag == f"{{{namespace['fb']}}}p"):
...
class ConversationViewSet(viewsets.ModelViewSet):
    ...
    def send_message(self, request, pk=None):
        ...
        try:
            ...
            return Response({'user_message': MessageSerializer(user_msg).data, 'ai_response': MessageSerializer(ai_msg).data,
            })
...
class TranslateView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response({'success': False, 'error': _('Внутренняя ошибка сервера')}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
