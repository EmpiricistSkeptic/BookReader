...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS("Starting deep inspection of all dictionary words..."))
        self.stdout.write("Format: ID | 'Word' | Byte Representation | Unicode Name of each character")
        self.stdout.write("---------------------------------------------------------------------")
        ...
        self.stdout.write("---------------------------------------------------------------------")
        if found_issues > 0:
            self.stdout.write(self.style.ERROR(f"Inspection complete. Found {found_issues} suspicious entries."))
        else:
            self.stdout.write(self.style.SUCCESS("Inspection complete. No suspicious characters found in words."))