...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        duplicates = (
            ...
            .filter(
                word_count__gt=1
            )  # Оставляем только те, что встречаются больше одного раза
            .order_by(
                "-word_count"
            )  # Сортируем, чтобы самые частые дубликаты были вверху
        )

        if not duplicates:
            self.stdout.write(
                self.style.SUCCESS("No duplicate words found. The database is clean.")
            )
            return

        self.stdout.write(
            self.style.WARNING(f"Found {len(duplicates)} words with duplicate entries:")
        )
        self.stdout.write("------------------------------------")
