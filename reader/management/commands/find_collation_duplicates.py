...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            "Searching for collation duplicates (case-insensitive, trimmed)..."
        )
        ...
        duplicates = (
            DictionaryEntry.objects.annotate(word_normalized=Trim(Lower("word")))
        ...
        if not duplicates:
            self.stdout.write(
                self.style.SUCCESS(
                    "No collation duplicates found. This is very strange."
                )
            )
            return
        ...
        self.stdout.write(
            self.style.WARNING(
                f"Found {len(duplicates)} groups of collation duplicates:"
            )
        )
        self.stdout.write("------------------------------------")
        ...
        for group in duplicates:
            ...
            entries = (
                DictionaryEntry.objects.annotate(word_normalized=Trim(Lower("word")))
                .filter(word_normalized=normalized_word)
                .order_by("id")
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Search complete. These are the words causing unstable sorting."
            )
        )
